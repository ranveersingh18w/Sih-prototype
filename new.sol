 ...
 * and supports storing an image (IPFS hash) + GPS location (lat/long).
 */
contract SupplyChainTrace {
    ...
    struct BlockEntry {
        ...
        string cropName;
        uint256 quantity;
        string location; // textual location
        string batchId;
        uint256 prevIndex;
        uint256 createdAt;
        // New fields for traceability
        string ipfsHash; // image stored on IPFS
        string latitude; // precise latitude
        string longitude; // precise longitude
    }
    ...
    modifier onlyRegisteredActor(string memory actorId, Role expectedRole) {
        ...
        require(
            actors[actorId].role == expectedRole,
            "Actor role does not match"
        );
        _;
    }
    ...
    // MODIFIED: This function now accepts the IPFS hash, latitude, and longitude
    function createFarmerBlock(
        ...
        external
        onlyRegisteredActor(farmerId, Role.Farmer)
        returns (uint256 index, string memory batchId)
    {
    ...
    function createCollectorBlock(
        ...
        external
        onlyRegisteredActor(collectorId, Role.Collector)
        returns (uint256)
    {
        require(
            prevIndex > 0 && prevIndex < blocks.length,
            "Previous index out of range"
        );
        return
            _createBlock(
                Role.Collector,
                ...
                batchId,
                prevIndex,
                "", // No IPFS Hash for Collector
                "", // No Lat for Collector
                "" // No Long for Collector
            );
    }
    ...
    function createManufacturerBlock(
        ...
        external
        onlyRegisteredActor(manufacturerId, Role.Manufacturer)
        returns (uint256)
    {
        require(
            prevIndex > 0 && prevIndex < blocks.length,
    ...
    function createDistributorBlock(
        ...
        external
        onlyRegisteredActor(distributorId, Role.Distributor)
        returns (uint256)
    {
        require(
            prevIndex > 0 && prevIndex < blocks.length,
    ...
    function getFullChainByBatch(
        string memory batchId
    ) public view returns (BlockEntry[] memory) {
