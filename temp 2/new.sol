 ...
 * This version adds Actor profiles and batch tracking by actor.
 */
contract SupplyChainTrace {
    ...
    // New mappings to track batches per actor
    mapping(string => string[]) private actorToBatchIds; // actorId -> [batchId1, batchId2, ...]
    mapping(string => mapping(string => bool)) private actorHasBatch; // actorId -> batchId -> bool
    ...
    function _createBlock(
        ...
        // Add batch to actor's list if it's not already there
        if (!actorHasBatch[actorId][batchId]) {
            actorToBatchIds[actorId].push(batchId);
            actorHasBatch[actorId][batchId] = true;
        }
    ...
    /**
     * @dev NEW FUNCTION: Get all batch IDs associated with a specific actor.
     * @param actorId The ID of the actor (e.g., "farmer-1").
     * @return An array of batch ID strings.
     */
    function getBatchesByActor(string memory actorId) public view returns (string[] memory) {
        return actorToBatchIds[actorId];
    }
}
